    ...
    public class AuthenticationHandlerFactory : IAuthHandlerFactory
    {
        ...
        public IAcquireTokenHandler Get(string name)
        {
            ...
            switch (config.AuthenticationConfig.AuthenticationTypeAsEnum)
            {
                ...
                default:
                    throw new ArgumentOutOfRangeException(nameof(config.AuthenticationConfig.AuthenticationType), $"unknown configuration type of {config.AuthenticationConfig.AuthenticationType}");
            }
        }
    }
}
