...
{
    public class ClientFactoryEvent: TelemetryEvent
    {
        public string Message { get; }
        public ClientFactoryEvent(string message)
        {
            Message = message;
        }
    }
    ...
    public class CosmosDbClientFactory : ICosmosDbClientFactory, IDisposable
    {
            ...
            lock (_sync)
            {
                ...
                var databaseTasks = config.Databases
                    .Select (kv => client.CreateDatabaseIfNotExistsAsync (
                        new Microsoft.Azure.Documents.Database { Id = kv.Key },
        ...
        {
            _bb.Publish (new ClientFactoryEvent (message));
        }
    }
}
